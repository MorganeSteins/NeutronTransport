...
point step_homog_unif(double x, double mu, int max_iter){
    int stop = 1;
    int step = 0 ;
    point new_point ;
    while (step < max_iter && stop > 0){
        new_point = deplacement_x(point(x, mu));
        if (new_point.get_x() <= 0 || new_point.get_x() >= 1) stop = 0 ;
        else stop = do_I_stop(point::sigmaS, point::sigmaA);
        step ++ ;
    }
    return new_point ;
}
