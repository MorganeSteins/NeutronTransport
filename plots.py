...
# phi_exact = np.array([density_no_scattering_homog_unif(x[i]+dx/2,1,sigmaT) for i in range(len(x))])
...
plt.legend()
plt.title("Simulation sur "+str(n)+" intervalles avec $N=10^5$ particules")
plt.show()
