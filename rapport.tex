...
\begin{figure}[ht!]
	...
	\begin{subfigure}{.49\textwidth}
		\centering
		\includegraphics[scale=0.42]{Figures/q4_ic_n1e4_nb25_2.png}
...
%Ceci entraîne immédiatement que les $\epsilon^n$ sont solution de la même équation que les $\pp^n$.
...
\vspace{0.2cm}
En fixant $\mu$, on utilise ce processus une unique fois et on obtient $\psi_t$ pour tous les intervalles souhaités. On peut regarder la solution pour différents $\mu$ et regarder l'erreur en fonction de la discrétisation spatiale. En effet on commet une erreur en assimilant $\pp$ à une fonction affine par morceaux, erreur qui diminue cependant avec $dx$ la largeur des intervalles comme sur la figure (\ref{fig:q9.errl2}). On remarque cependant que pour $N>500$ l'erreur ne décroît plus. Nous supposons que des erreurs numériques sont à l'origine de cette stagnation de l'erreur.
...
\subsection{Matériau diffusant}
On se place maintenant dans le cas d'un matériau diffusant. Nous ne pouvons plus maintenant nous placer à $\mu$ fixé et regarder $\pp(x,\mu)$ mais seulement la valeur moyennée en angle $\ppt(x)$. Il faut donc discrétiser l'espace $[-1;1]$ des angles possibles et sommer les contributions de chaque arc de cercle.
...
\begin{equation}
	\frac{-1}{3} \ppt_0(x)'' = 1
\end{equation}
soit $\ppt_0(x)$ un polynôme d'ordre 2 en $x$ de la forme $\ppt_0(x) = -3x^2+bx+c$. Pour déterminer les coefficients on utilise la symétrie de la solution par rapport à la droite $x=\frac{1}{2}$ qui donne $b=3$. L'ordonnée à l'origine est donnée par les conditions aux limites de flux entrant nul.
...
(\ref{eq.transport}) par une équation de diffusion de la forme :
\begin{equation}
...
de Cholesky. Le choix de cette méthode n'est pas anodin : la matrice
$\mathbb{A}$ ne change pas au cours des itérations. Ainsi, la résolution de ce
système est effectuée en $\mathcal{O} (n^2)$ opérations.
