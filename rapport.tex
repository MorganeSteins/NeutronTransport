...
\begin{equation}
    ...
    \mu \frac{\partial \pp}{\partial x} (x,\mu) + \st \pp(x,\mu) = \delta_0(x)
\end{equation}
avec pour condition aux limites un flux entrant nul, c'est à dire 
$$ \pp(0,\mu >0) = \pp(1,\mu<0) = 0 $$ 
...
\begin{align*}
    \int_0^x \left( \mu \frac{\partial \pp}{\partial x} (t,\mu)  + \st
    \pp(t,\mu)\right) e^\frac{\st t}{\mu} dt &= \int_0^x \delta_0(t) dt \\
    \iff  \mu \pp(x,\mu) e^{\frac{\st x}{\mu}} &= \mu \pp(0,\mu) + 1 \\
    \iff  \pp(x,\mu) &= \frac{1}{\mu} e^{-\frac{\st x}{\mu}}
\end{align*}
...
Ceci donne alors une distributions de neutrons avec une densité $f$ égale au libre parcours.

\begin{equation*}
    f(x_1) = \frac{\st}{\mu}e^{-\frac{\st}{\mu}x_1} = \st \pp(x_1,\mu)
\end{equation*}
...
\begin{equation}
    \pp(x,\mu) = \frac{\PP(x_1 \in [x,x+dx])}{\st dx}
\end{equation}

En approximant la probabilité par la fréquence lors d'un grand nombre de tirage, nous sommes bien capables de trouver $\pp(x,\mu)$ pour tout $x$.


\vspace{0.5cm}
\subsubsection{Source uniforme}
On s'intéresse ensuite au cas d'une source $S$ uniforme et unitaire sur tout l'intervalle $[0,1]$.
Comme on écrit $S(x) = \int_0^1 S(t) \delta_t(t)dt$ on peut réutiliser l'étude faite avec une source ponctuelle. Cette source peut maintenant être située en tout point de l'intervalle d'étude. 
Par translation on obtient la solution de l'équation pour une source ponctuelle en $x_0$ pour $\mu>0$
\begin{equation}
    \label{eq:sol.point.x0}
    \pp_{x_0}(x,\mu) = 1_{(x>x_0)}\frac{1}{\mu} e^{-\frac{\st (x-x_0)}{\mu}} 
\end{equation}
Par symétrie on trouve la solution pour $\mu<0$ en changeant simplement l'indicatrice $1_{(x<x_0)}$.

\vspace{0.1cm}
Par linéarité la solution de notre equation pour $S=1$ 
\begin{equation*}
    \pp(x,\mu) = \int_0^1 \pp_{x_0}(x,\mu)\ dx_0
\end{equation*} 
...
\begin{equation*}
    f_{\mu_0}(x_1) = \int_0^1 f_{\mu_0}(x_1|x_0)f_{\mu_0}(x_0)\ dx_0
\end{equation*}

$f_{\mu_0}(x_1|x_0)$ est donnée par la solution (\ref{eq:sol.point.x0}) qui est le libre parcours depuis $x_0$ à $\st$ près et $f_{\mu_0}(x_0)$ par la densité de la source, ici $1$.
On obtient la densité $f(x_1)$
$$ f_{\mu_0}(x_1) = \st \int_0^1 \pp_{x_0}(x,\mu) \ dx  = \st \pp(x,\mu)$$
...
On a alors
$$ f(x_1) = \frac{1}{2} \into f_{\mu_0}(x_1) \ d\mu = \frac{\st}{2} \into \pp(x_1,\mu) \ d\mu  = \frac{\st}{2} \tilde{\pp}(x) $$

Nous sommes donc capable d'approximer $\pp$ mais aussi $\tilde{\pp}$ par méthode Monte-Carlo

\begin{equation}
    \pp(x,\mu) = \frac{\PP(x_1 \in [x,x+dx]| \mu_0)}{\st dx} \hspace{1cm} \tilde{\pp}(x) = \frac{\PP(x_1 \in [x,x+dx])}{2 \st dx}
\end{equation}
...
Nous utilisons la suite des flux $n$ fois collisionnés $\pp^n$ et les différences entre deux itérations $\epsilon = \pp^n-\pp^{n-1}$ avec $\epsilon^0 = \pp^0$.
Les flux collisionnés vérifient l'équation
\begin{equation}
    \label{eq:diffusion}
    \mu \frac{\partial \pp^n}{\partial x} (x,\mu) + \st \pp^n(x,\mu) = \sa S(x,\mu) + \frac{\Ssss}{2} \into \pp^{n-1}(x,\mu')\ d\mu' 
\end{equation}

Pour simplifier l'étude on considère ici que l'absorbtion est négligeable, i.e. $\st = \Ssss$.
