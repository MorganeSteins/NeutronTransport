...
\vspace{0.1cm}
La méthode Monte-Carlo consiste à simuler un grand nombre de trajectoires de
neutrons et en déduire le flux neutronique par la loi des grands nombres, ce qui
...
Pour simuler ce flux neutronique avec un solveur Monte-Carlo sur un intervalle de
...
on va procéder par intervalle de fluctuation à 95\%. L'intervalle de fluctuation à 95\% donne l'intervalle de valeurs dans lequel 95\% de nos mesures doivent être, en postulant que la théorie (notre code est juste) est vraie.
...
Ainsi, l'espérance et la variance de $X$ sont $\mathbb{E} ( X ) = N
	p_{i|\mu_0}$ et $\mathbb{V}\mathrm{ar} (X) = N p_{i|\mu_0} (1-p_{i|\mu_0})$.

Pour une seule particule, on peut faire les estimations suivantes :
\begin{equation*}
	\mathbb{E}_\varphi = \frac{1}{\Delta x} \int_{I_i} \psi(x,\mu_0) dx = \frac{\mathbb{E} (X)}{N\Delta x }
\end{equation*}
et
\begin{equation*}
	\mathbb{V}\mathrm{ar}_\varphi = \frac{1}{\Delta x} \int_{I_i} (\psi(x,\mu_0)- \mathbb{E}_\varphi)^2 dx = \frac{\mathbb{V}\mathrm{ar}(X)}{N\Delta x}
\end{equation*}

Cette estimation nous donne un intervalle asymptotique de confiance :
\begin{equation*}
	I_c^i = \left[\psi - 1.96 \sqrt{\frac{\mathbb{V}\mathrm{ar}(X)}{N\Delta x}} , \psi + 1.96 \sqrt{\frac{\mathbb{V}\mathrm{ar}(X)}{N\Delta x}} \right]
\end{equation*}
...
évoluant en l'inverse de $\sqrt{N}$, il est ici de largeur de l'ordre de
$10^{-3}/25$ pour $25$ intervalles. Les points donnent toutefois l'impression
d'être compris dedans de manière très qualitative. Nous avons tracé l'intervalle
de confiance à $95\%$ pour une variable aléatoire suivant une loi gaussienne
puisque par théorème central limite la moyenne empirique que nous calculons tend
vers l'espérance de la valeur ponctuelle de $\pp$.
...
On est donc capable d'écrire la solution théorique dont on voit deux allures pour deux valeurs de $\mu$ différentes sur la figure (\ref{fig:q5.allures1}).
\begin{figure}[htp!]
	\centering
	\includegraphics[scale=0.5]{Figures/q6_sol_theorique.png}
	\caption{$\pp(x,\mu)$ théorique dans le cas $\st$ non homogène}
	\label{fig:q5.allures1}
\end{figure}

Pour calculer la solution avec une méthode Monte-Carlo il faut mettre en oeuvre la méthode de Woodcock qui permet de prendre en compte les variations de $\st$.
...
Contrairement aux cas précédents on ne connaît pas la solution analytique, mais on sait que la solution doit être symétrique par rapport à $x=1/2$. Il serait possible de tester que notre code fonctionne bien en prenant une solution manufacturée. Il faudrait alors déterminer la source à l'origine de ce flux neutronique puis tirer les particules de départ selon la densité de cette source et enfin comparer les résultats. 
...
Il est important de noter que pour $N=10^7$ particules le temps de calcul est déjà de plusieurs secondes. Augmenter le nombre d'intervalles et donc $N$ pour conserver un intervalle de confiance de même largeur ferait donc rapidement exploser les temps de calcul. 
...
On en déduit ensuite $\ppb_i$ grâce à la relation de fermeture. C'est la
grandeur à laquelle nous allons nous intéresser car c'est l'unique que l'on peut
calculer quand il y a de la dispersion, et nous voulons un solveur général pour
tous les cas physiques. Ce que nous appelons donc la \textit{solution calculée}
i.e. la sortie de notre algorithme est l'approximation constante par morceaux
$\pp(x \in [(i-1)dx;idx],\mu \text{ fixé }) = \ppb_i$.
...
\vspace{0.2cm}
Dans ce cas-ci nous n'avons pas la solution analytique de l'équation. Une
méthode de vérification est toutefois de comparer qualitativement avec la
solution obtenue dans le cas Monte-Carlo. Parler de convergence d'une solution
vers l'autre alors qu'elles sont toutes les deux approchées n'est pas envisagé.
La figure (\ref{fig:q12.MC.deter}) montre les résultats par simulation
Monte-Carlo et déterministe. Nous avons fixé $\eta=10^{-9}$ et l'algorithme
avait convergé en $46$ itérations. La courbe déterministe est bien plus lisse
que la courbe M0nte-Carlo et obtenue en un temps bien inférieur :  3 secondes
pour Monte-Carlo et 0.2 secondes pour le code déterministe.

\begin{figure}
	\centering
	\includegraphics[scale=0.42]{Figures/q12_comp.png}
	\caption{Cas purement diffusant : solutions Monte-Carlo et déterministe}
...
On prend $N_\mu=10^4$ de référence et on regarde pour des $N_\mu$ au moins $10$ fois plus petits. On remarque d'abord que le nombre d'itérations reste globalement le même. On a ensuite une convergence de cette pseudo-erreur comme visible sur la figure (\ref{fig:q12.Nmu}). Il nous est difficile d'expliquer la pente de $\frac{3}{2}$ mais le fait que l'erreur diminue rapidement quand $N_\mu$ augmente est le comportement attendu.

\begin{figure}[ht!]
	\centering
	\includegraphics[scale=0.42]{Figures/q12_errmu.png}
	\caption{Erreur en fonction de $N_\mu$ par rapport à la solution à $N_\mu=10^4$}
	\label{fig:q12.Nmu}
\end{figure}
...
On regarde la solution obtenue pour plusieurs $\epsilon$ ainsi que le nombre d'itérations nécessaires pour converger à $\nu=10^{-7}$ près avec $N_x=1000$ points en $x$ et $N_\mu=100$ arcs de cercles. Nous avons choisit ce paramètre de manière à avoir une solution la plus précise possible dans des temps de calcul raisonnables.

\begin{table}[h]
	\centering
	\begin{tabular}{ccccc}\toprule
		$\bm{\epsilon}$              & 1     & 0.1   & 0.01  & 0.001 \\ \midrule
		\textbf{Nombre d'itérations} & 32    & 505   & 25243 & ?     \\
		\textbf{Temps d'exécution}   & 0.09s & 1.18s & 1m    & >20m  \\\bottomrule
	\end{tabular}
\end{table}


Le nombre d'itérations nécessaires pour arriver à la précision souhaitée
augmente très rapidement avec $\epsilon$ qui diminue, tout comme le temps
d'exécution. On voit donc bien l'intérêt de mettre en place une méthode
d'accélération comme par exemple l'accélération par diffusion synthétique. Nous
avons laissé tourner le code 20 minutes pour $\epsilon=0.001$ avant de couper
l'exécution et nous n'avions toujours pas convergé.

Avant de passer à l'accélération regardons l'impact des paramètres $N_x$ et
$N_\mu$. Comme le nombre de boucle augmente linéairement avec $N_\mu$ et $N_x$,
multiplier l'un ou l'autre par 10 multiplie d'autant le nombre d'opérations et
donc le temps de calcul. Comme le nombre d'itérations nécessaires avec
$\epsilon$ faible augmente très vite aussi, on se retrouve vite avec de grands
temps de calcul en comparaison avec avant.

De même que dans les questions précédentes $N_x$ et $N_\mu$ améliorent la
précision, mais comme il n'y a pas de solution analytique on ne peut pas
calculer l'erreur commise. On peut toutefois regarder la convergence de la
solution obtenue vers la solution théorique en limite de diffusion quand on
diminue $\epsilon$.
\begin{figure}[ht!]
	\centering
	\includegraphics[scale=0.6]{Figures/q14_diffepsilon.png}
	\caption{Solution pour différents $\epsilon$}
	\label{fig:q12.eps}
\end{figure}

On a vu que sans accélération il est difficile de demander $\epsilon$ plus petit
que $0.01$ mais sur la figure (\ref{fig:q12.eps}) on voit que ceci ne permet pas
d'approcher le cas de la limite de diffusion avec une erreur $\mathcal{L}^2$
acceptable, on voit même l'erreur à l'œil nu.

Tout ceci justifie amplement la mise en place de l'accélération par diffusion synthétique.
...
\vspace{0.2cm}
On peut regarder les résultats de simulation et vérifier d'une part que la solution est la même qu'avant la mise en place de l'accélération par diffusion synthétique et d'autre part que celle-ci permet de prendre des $\epsilon$ plus petits.

C'est ce que montre la figure (\ref{fig:q16.eps}), on a pu calculer $\epsilon=0.0001$ et la solution est confondue avec la limite de diffusion.


\begin{figure}[ht!]
	\centering
	\includegraphics[scale=0.65]{Figures/q16_diffepsilon.png}
	\caption{Solution pour différents $\epsilon$}
	\label{fig:q16.eps}
\end{figure}
...
L'accélération synthétique permet d'accélérer\footnote{heureusement} grandement
le calcul dans le cas de la limite de diffusion comme cela peut être vu dans le
tableau ci-dessous.

\begin{table}[h]
	\centering
	\begin{tabular}{cccc}\toprule
		$\bm{\epsilon}$ & 1 (sans absorption) & 0.1 & 0.01  \\\midrule
		Sans DSA        & 32                  & 505 & 25243 \\
		Avec DSA        & 24                  & 27  & 23    \\\bottomrule
	\end{tabular}
	\caption{Nombre d'itération avec et sans accélération. Les paramètres de simulations sont $\nu = 10^{-7}$, $N_x = 1000$ points, et $N_\mu = 100$ arcs de cercles.}
\end{table}
...
Nous avons pu comparer au cours de ce projet deux méthodes très différentes :
une méthode Monte-Carlo et une méthode déterministe.

Chacune des deux méthodes a des avantages et des inconvénients. La méthode
Monte-Carlo permet par exemple de simuler le problème sans discrétisation en
angle et se généralise bien en 3D mais la méthode converge relativement
lentement.

Tandis que la méthode
déterministe plus rapide dans notre cas et converge mieux mais doit malgré tout être accélérée
artificiellement et nécessite de discrétiser en angle, ce qui peut rapidement
devenir problématique en 3D.
