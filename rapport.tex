...
\begin{equation}
    \psi(x,\mu) = \lim\limits_{dx \rightarrow 0} \frac{\PP(x_1 \in [x,x+dx] | \mu)}{\st dx}
\end{equation}
...
Empiriquement, s'il est trop grand l'approximation de l'intégrale est fausse et s'il est trop petit il n'y aura aucune trajectoire qui mène à l'intervalle $[x;x+dx]$.
Nous avons choisit ici de prendre $dx=\frac{1}{\sqrt(N)}$ après quelques essais.

\begin{figure}[htp!]
    \centering
    \begin{subfigure}{.49\textwidth}
        \centering
        \includegraphics[scale=0.45]{Figures/sol4_et_simulations_moyennes.png}
        \caption{Solution et simulations pour $\mu=1$}
        \label{fig:q4.sol}
    \end{subfigure}
    \begin{subfigure}{.49\textwidth}
        \centering
        \includegraphics[scale=0.45]{Figures/Q3_vitessesL2_Linfty_cv_025.png}
